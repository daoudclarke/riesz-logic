...
\begin{proof}[MP]
MP+ follows trivially from the assumption of the rule MP in RL.
\end{proof}

%% 1 P(x) & P(x => y) -> P(y) # label(MP) # label(non_clause).
%% [assumption].
%% 2 P(x) -> P(x v 0) # label(BALPI) # label(non_clause) # label(goal).
%% [goal].
%% 3 -P(x) | -P(x => y) | P(y) # label(MP).  [clausify(1)].
%% 6 P(x => x v y) # label(R2).  [assumption].
%% 13 P(c1) # label(BALPI).  [deny(2)].
%% 14 -P(c1 v 0) # label(BALPI) # answer(BALPI).  [deny(2)].
%% 45 P(c1 v x).  [hyper(3,a,13,a,b,6,a)].
%% 46 $F # answer(BALPI).  [resolve(45,a,14,a)].
...
%% 1 P(x) & P(x => y) -> P(y) # label(MP) # label(non_clause).
%% [assumption].
%% 2 P((x => 0) v 0 => 0) -> P(x) # label(POS) # label(non_clause) #
%% label(goal).  [goal].
%% 3 -P(x) | -P(x => y) | P(y) # label(MP).  [clausify(1)].
%% 4 P((x => y) => ((y => z) => (x => z))) # label(R1a).  [assumption].
%% 5 P(((x => y) => (z => y)) => (z => x)) # label(R1b).  [assumption].
%% 6 P(x => x v y) # label(R2).  [assumption].
%% 13 P((c1 => 0) v 0 => 0) # label(POS).  [deny(2)].
%% 14 -P(c1) # label(POS) # answer(POS).  [deny(2)].
%% 17 P(((x => y) => y) => x).  [hyper(3,a,4,a,b,5,a)].
%% 22 P((x v y => z) => (x => z)).  [hyper(3,a,6,a,b,4,a)].
%% 172 P((c1 => 0) => 0).  [hyper(3,a,13,a,b,22,a)].
%% 181 P(c1).  [hyper(3,a,172,a,b,17,a)].
%% 182 $F # answer(POS).  [resolve(181,a,14,a)].
